...
class RegisterForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        if commit:
            user.save()  # Signal akan create Profile di sini
            
            role = self.cleaned_data['role']
            is_admin_user = (role == 'admin')
            
            # Gunakan get_or_create untuk handle jika signal gagal atau sudah ada
            ...
            # Jika profile sudah ada sebelumnya, update rolenya
            if not created:
                profile.role = role
